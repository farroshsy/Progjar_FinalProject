...
class ChatClient:
        ...
        try:
            ...
            if command == "register":
                ...
                return self.send_private_realm_message(
                    realm_name, username_dest, message
                )
                ...
                file_path = j[3].strip()
                return self.send_group_file_to_realm(
                    realm_name, usernames_to, file_path
                )
                ...
                realm_name = j[1].strip()
                return self.get_realminbox(self.tokenid, realm_name)
    ...
    def send_private_realm_message(self, realm_name, username_dest, message):
        ...
        string = "sendprivaterealm {} {} {} {}\r\n".format(
            self.tokenid, realm_name, username_dest, message
        )
        result = self.sendstring(string)
        if result["status"] == "OK":
    ...
    def send_file_to_realm(self, realm_name, username_dest, file_path):
        ...
        string = "send_file_realm {} {} {} {} {}\r\n".format(
            self.tokenid, realm_name, username_dest, file_path, encoded_content
        )
        result = self.sendstring(string)
        if result["status"] == "OK":
    ...
    def send_group_realm_message(self, realm_name, usernames_to, message):
        ...
        string = "send_group_realm {} {} {} {}\r\n".format(
            self.tokenid, realm_name, ",".join(usernames_to), message
        )
        result = self.sendstring(string)
        if result["status"] == "OK":
            return "Message sent to group {} in realm {}".format(
                ",".join(usernames_to), realm_name
            )
        else:
            return "Error: {}".format(result["message"])
    ...
    def send_group_file_to_realm(self, realm_name, usernames_to, file_path):
        ...
        if result["status"] == "OK":
            return "File sent to group {} in realm {}".format(
                ",".join(usernames_to), realm_name
            )
        else:
            return "Error: {}".format(result["message"])
