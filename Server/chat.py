...
class Chat:
        ...
        try:
            ...
            if command == "register":
                ...
                return self.send_group_message(
                    sessionid, usernamefrom, group_usernames, message
                )
                ...
                username = j[1].strip()
                logging.warning(
                    "GETPRESENCE: get presence for user {}".format(username)
                )
                ...
                logging.warning(
                    "DELETELASTMESSAGE: delete last message for user {}".format(
                        username
                    )
                )
                return result
                ...
                logging.warning(
                    "FORWARDMESSAGE: forward message from {} to {}".format(
                ...
                logging.warning(
                    "REPLYMESSAGE: reply message for user {}".format(username)
                )
                return result
                ...
                message = " ".join(j[4:])
                logging.warning(
                    "SENDPRIVATEREALM: session {} sends a message from {} to {} in realm {}".format(
                        sessionid,
                ...
                return self.send_private_realm_message(
                    sessionid, realm_name, username_dest, message
                )
                ...
                message = " ".join(j[4:])
                logging.warning(
                    "RECVREALMPRIVATEMSG: received message from {} to {} in realm {}".format(
                        username_from, username_dest, realm_name
                    )
                )
                return self.recv_realm_message(
                    realm_name, username_from, username_dest, message, data
                )
                ...
                logging.warning(
                    "SENDFILEREALM: session {} sends a file from {} to {} in realm {}".format(
                ...
                encoded_file = j[5].strip()
                logging.warning(
                    "RECVFILEREALM: received file from {} to {} in realm {}".format(
                ...
                username_depart = self.sessions[sessionid]["username"]
                logging.warning(
                    "SENDGROUPFILEREALM: session {} sends a file from {} to {} in realm {}".format(
                ...
                message = " ".join(j[4:])
                logging.warning(
                    "RECVGROUPREALM: received message from {} to {} in realm {}".format(
                        username_depart, usernames_dest, realm_name
                    )
                )
                return self.recv_group_realm_message(
                    realm_name, username_depart, usernames_dest, message, data
                )
                ...
                encoded_file = j[5].strip()
                logging.warning(
                    "RECVGROUPFILEREALM: received file from {} to {} in realm {}".format(
                        username_depart, usernames_dest, realm_name
                    )
                )
                ...
                realm_name = j[1].strip()
                logging.warning(
                    "GETREALMINBOX: {} from realm {}".format(tokenid, realm_name)
                )
                return self.get_realminbox(tokenid, realm_name)
    ...
    def send_file(
        ...
        inqueue_receiver.setdefault(username_from, Queue()).put(filename)
        inqueue_receiver.setdefault(username_from, Queue()).put(encoded_file)
    ...
    def send_group_file(
        ...
        for username_dest in usernames_dest:
            ...
            outqueue_sender.setdefault(username_from, Queue()).put(json.dumps(message))
            inqueue_receiver.setdefault(username_from, Queue()).put(json.dumps(message))
    ...
    def recv_realm(self, realm_name, ipaddress_dest, port_dest, data):
        ...
        if realm_thread.connect():
            return {
                "status": "OK",
                "message": "Realm received and connected successfully",
            }
        else:
            del self.realms[realm_name]
            return {"status": "ERROR", "message": "Failed to connect to the realm"}
    ...
    def send_private_realm_message(self, sessionid, realm_name, username_dest, message):
        print(realm_name, self.realms)
        if sessionid not in self.sessions:
            return {"status": "ERROR", "message": "Session Tidak Ditemukan"}
        if realm_name not in self.realms:
            return {"status": "ERROR", "message": "Realm Tidak Ada"}
    ...
    def recv_realm_message(
        self, realm_name, username_depart, username_dest, message, data
    ):
        if realm_name not in self.realms:
    ...
    def recv_file_realm(
        self, realm_name, username_depart, username_dest, file_path, encoded_file, data
    ):
        if realm_name not in self.realms:
            return {"status": "ERROR", "message": "Realm Tidak Ditemukan"}
        s_fr = self.get_user(username_depart)
    ...
    def recv_group_realm_message(
        self, realm_name, username_from, usernames_to, message, data
    ):
        if realm_name not in self.realms:
            return {"status": "ERROR", "message": "Realm Tidak Ditemukan"}
    ...
    def recv_group_file_realm(
        self, realm_name, username_from, usernames_to, file_path, encoded_file, data
    ):
        if realm_name not in self.realms:
            return {"status": "ERROR", "message": "Realm Tidak Ditemukan"}
        s_fr = self.get_user(username_from)
    ...
    def get_realminbox(self, tokenid, realm_name):
        if not self.is_authorized(tokenid):
            return {"status": "ERROR", "message": "Not authorized"}

        print(username, realm)
        print(self.realms)
        username = self.get_username(tokenid)
        realm = self.realms.get(realm_name)
        if not realm:
            return {"status": "ERROR", "message": "Realm not found"}

        messages = realm.get_inbox(username)
        ...
        if messages["status"] == "OK":
            ...
            for user, user_msgs in msgs.items():
                output += "User: {}\n".format(user)
            ...
            return "Error: {}".format(messages["message"])
